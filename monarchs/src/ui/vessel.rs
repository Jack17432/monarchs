...
pub fn vessel_switch_system(
    ...
    let vessels = vessels.iter().filter(|(entity, _)| soul.2.0.contains(entity)).collect::<Vec<_>>();
        ...
        table.header(20.0, |mut header| {
            ...
            for (entity, name) in vessels {
                body.row(20.0, |mut row| {
                    ...
                    if entity == soul.1.0 {
                        row.col(|mut ui| {ui.label("*");});
                    } else {
                        row.col(|mut ui| {ui.label("");});
                    }

                    let name = name.get_name();
                    row.col(|mut ui| {ui.label(name);});
                });
            }
        });
    });
